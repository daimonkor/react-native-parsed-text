  ...
  parse() {
    ...
    this.patterns.forEach((pattern) => {
      ...
      parsedTexts.forEach((parsedText) => {
        ...
        while (textLeft) {
          let matches = textLeft.match(pattern.pattern);

          if (!matches) { break; }

          matches = [matches[0]];

          let index = textLeft.indexOf(matches);

          let previousText = textLeft.substr(0, index);
          indexOfMatchedString += index;

          parts.push({children: previousText});

          parts.push(this.getMatchedPart(pattern, matches[0], matches, indexOfMatchedString));

          textLeft = textLeft.substr(index + matches[0].length);
